...
namespace SerialAssistant
{
    public partial class Form1 : Form
    {
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            int count=0;
            bool state=true;
            double anglex, anglez,acc;
            double ag;
            while (true)
            {

                //textBox_udp.Text = socketudp.Udp_date;
                textBox_udp.Text = "250";
                string[] sArray = Regex.Split(socketudp.Udp_date, ",", RegexOptions.IgnoreCase);
                Console.WriteLine(sArray[6]);
                Console.WriteLine(sArray[7]);
                Console.WriteLine(sArray[8]);
                Console.WriteLine(count++);

                
                double.TryParse(sArray[1], out acc);
                if (Math.Abs(acc) > 1.2)
                {
                    if(state){ serialPort1.Write("#005P1000T200!\r\n"); state = false; }
                    else{ serialPort1.Write("#005P2000T200!\r\n"); state = true; }
                    System.Threading.Thread.Sleep(1000);

                }
                else
                {
                    double.TryParse(sArray[6], out anglex);
                    double.TryParse(sArray[8], out anglez);

                    ag = (double)anglex * 7.4;
                    anglex = (int)ag + 950;
                    ag = (double)anglez * 5;
                    anglez = (int)ag + 1500;

                    if (anglex < 500) anglex = 500;
                    if (anglex > 2500)anglex = 2500;
                    if (anglez < 500) anglez = 500;
                    if (anglez > 2500) anglez = 2500;
                    //textBox_send.Text = sArray[6]+anglex.ToString()+ count.ToString();
                    serialPort1.Write("#002P" + anglex.ToString() + "T200!\r\n");
                    System.Threading.Thread.Sleep(100);
                    serialPort1.Write("#000P" + anglez.ToString() + "T200!\r\n");
                ...
                if (serialPort1.IsOpen)
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                           ...
                           // while (true)
                           // {
                                serialPort1.Write(socketudp.Udp_date);
                               // textBox_send.Text = socketudp.Udp_date;
                                textBox_udp.AppendText(socketudp.Udp_date);
                                num = socketudp.Udp_date.Length;
                           // }
                        }
                    }
        ...
        {
            socketudp.StartService();
            socketudp.StopAccept();
            System.Threading.Thread.Sleep(200);
            socketudp.StartAccept();
